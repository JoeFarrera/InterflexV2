...
public class PanelSgavmovsusuariresumView1 extends SgaJUPanel
{
  // Panel binding definition used by design time
  private JUPanelBinding panelBinding = new JUPanelBinding("PanelSgavmovsusuariresumView1UIModel");
...
// Fields for attribute:  SgavmovsusuariresumView1

  private SgaJTable tableSgavmovsusuariresumView1 = new SgaJTable();
...
// Fields for attribute:  Fecha fin
   ...
   */
  public PanelSgavmovsusuariresumView1()
  {
  }
  ...
  public void jbInit() throws Exception
  {
    ...
    // Code support for view object:  SgavmovsusuariresumView1
    tableSgavmovsusuariresumView1.setModel((TableModel)panelBinding.bindUIControl("SgavmovsusuariresumView1", tableSgavmovsusuariresumView1));
    tableSgavmovsusuariresumView1.setEditable(false);
    scroller.getViewport().add(tableSgavmovsusuariresumView1, null);
    ...
    navBar.setModel(JUNavigationBar.createViewBinding(panelBinding, navBar, "SgavmovsusuariresumView1", null, "SgavmovsusuariresumView1Iter"));
...
/*    add(filtroPanel, BorderLayout.NORTH);
  ...
  public static void main(String [] args)
  {
    ...
    PanelSgavmovsusuariresumView1 panel = new PanelSgavmovsusuariresumView1();
  ...
  public void setBindingContext(BindingContext bindCtx)
  {
    ...
    if (panelBinding.getPanel() == null)
    {
      ...
      panelBinding.findIterBinding("SgavmovsusuariresumView1Iter").getViewObject().setMaxFetchSize(0);
      panelBinding.refreshControl();
      try
      {
        jbInit();
        panelBinding.refreshControl();
        panelBinding.findIterBinding("SgavmovsusuariresumView1Iter").getViewObject().setMaxFetchSize(-1);
  ...
  private void jButtonSeleccionar_actionPerformed(ActionEvent e)
  {
    try
    {
      ...
      {
        ViewCriteria mViewCriteria = panelBinding.findIteratorBinding("SgavmovsusuariresumView1Iter").getViewCriteria();
        Row row = mViewCriteria.first();
        while (row != null)
        {
          row.remove();
          row = mViewCriteria.next();
        }
        
        row = mViewCriteria.createViewCriteriaRow();
        row.setAttribute("Data", "entre(" + mFecini.getField().trim() + ", " + mFecfin.getField().trim() + ")");
        mViewCriteria.insertRow(row);
        navBar.silentMode(mViewCriteria);
      }
    }
    catch(Exception ex)
    {
